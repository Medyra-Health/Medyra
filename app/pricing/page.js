...
import { FileText, Check, Loader2, ArrowLeft, Shield } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
      ...
      <div className="container mx-auto px-4 py-12 md:py-16">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-5xl font-bold mb-3">{t('pricing.title')}</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 max-w-7xl mx-auto">
          {pricingTiers.map((tier) => (
            <Card
              key={tier.key}
              className={`flex flex-col ${tier.highlighted ? 'border-blue-600 border-2 shadow-xl ring-1 ring-blue-600/20' : 'border-gray-200'}`}
            >
              <CardHeader className="pb-3">
                {tier.highlighted && (
                  <Badge className="mb-2 w-fit bg-blue-600">{t('pricing.mostPopular')}</Badge>
                )}
        ...
        <div className="flex flex-wrap justify-center gap-6 mt-10 text-sm text-gray-500">
          <div className="flex items-center gap-2">
            <Shield className="h-4 w-4 text-green-600" /> GDPR Compliant
          </div>
          <div className="flex items-center gap-2">
            <Check className="h-4 w-4 text-green-600" /> Cancel anytime
          </div>
          <div className="flex items-center gap-2">
            <Check className="h-4 w-4 text-green-600" /> No hidden fees
          </div>
          <div className="flex items-center gap-2">
            <Check className="h-4 w-4 text-green-600" /> Data deleted after 30 days
          </div>
        </div>
            ...
            {[
              {
                ...
                a: 'Yes. All data is encrypted in transit and at rest. We are GDPR compliant and automatically delete your reports after 30 days. We never sell or share your data.'
              },
              {
                q: 'Can I cancel anytime?',
                a: 'Absolutely. Cancel your subscription at any time from your dashboard — no penalties, no questions asked.'
              },
              {
                q: 'Is this medical advice?',
                a: 'No. Medyra is an educational tool that helps you understand medical terminology. Always consult your doctor for medical decisions.'
              },
              {
                q: 'What file formats are supported?',
                a: 'PDF, JPG, PNG, and TXT. Our OCR technology can extract text from scanned images and photos of lab reports.'
              },
              {
                q: 'Which languages are supported?',
                a: 'Medyra is available in 16 languages including English, German, French, Spanish, Arabic, Chinese, Japanese, and more.'
              }
            ].map((item, i) => (
